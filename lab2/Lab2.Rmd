...
```
The interval band does not contain all the datapoints.
...
### (d)
If there is a higher order but we are more certain that higher order parameters are not needed we can set the omega-values high as it creates a stronger prior and the mu-prior values at 0. Which would mitigate the problem of unneccesary high order polynom.
